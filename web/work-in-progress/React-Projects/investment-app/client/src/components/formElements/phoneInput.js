...
//Components
import DialCodeReactSelect from "./dialCodeReactSelect";
...
const PhoneInput = (props) => {
  ...
  //Render...
  return (
    <div className={props.size} ref={phoneInput}>
      <DialCodeReactSelect parent={phoneInput} form={props.form} />
    </div>
  );
};
...
/*const TextInput = (props) => {
  ...
  const [style, setStyle] = useState({
    countryCodeBorderColour: "transparent",
    iconColour: "white",
  ...
  useEffect(() => {
    if (props.disabled) {
      setStyle((prevState) => {
        return {
          ...prevState,
          ...{ iconColour: "dimgrey", inputBorderColour: "dimgrey", labelColour: "dimgrey" },
        };
      });
    } else {
      setStyle((prevState) => {
        return {
          ...prevState,
          ...{ iconColour: "white", inputBorderColour: "black", labelColour: "white" },
        };
      });
    }
  }, [props.disabled]);
  ...
  const handleFocus = (e) => {
    if (e.target.name === "dialCode") {
      setStyle((prevState) => {
        return { ...prevState, ...{ countryCodeBorderColour: "rgba(255, 215, 0, 1)" } };
      });
    }
  ...
  const handleBlur = (e) => {
    if (e.target.name === "dialCode") {
      setStyle((prevState) => {
        return { ...prevState, ...{ countryCodeBorderColour: "transparent" } };
      });
    }
    if (e.target.name === "phoneNumber") {
      setStyle((prevState) => {
  ...
  const handleChange = (e) => {
    if (e.target.name === "dialCode") {
      let countryCode = e.target.selectedOptions[0].dataset.countrycode;
      let src = "https://flagpedia.net/data/flags/w580/" + countryCode.toLowerCase() + ".webp";
      setCountry((prevState) => {
        return { ...prevState, ...{ code: countryCode, flagSrc: src } };
      });

      let formattedDialCode = "+" + e.target.value;
      props.onChange({ dialCode: formattedDialCode });
    }
    if (e.target.name === "phoneNumber") {
      let formattedPhoneNumber = formatPhoneNumber(e.target.value);
      props.onChange({ phoneNumber: formattedPhoneNumber });
    }
  };
  ...
  return (
    <div className={props.size}>
      <div className="countryCodeBox" style={{ borderColor: style.countryCodeBorderColour }}>
        <select
          className="countryCode"
          style={props.disabled ? { cursor: "default" } : { cursor: "pointer" }}
          name="dialCode"
          value={props.dialCode.replace("+", "")}
          onFocus={handleFocus}
          onBlur={handleBlur}
        ...
        <div className="dropdownArrowBox">
          <IoIosArrowDown className="icon" style={{ color: style.iconColour }} />
        </div>
      </div>
      <div className="phoneNumberBox" style={{ borderColor: style.inputBorderColour }}>
        <div className="dialCode" style={props.disabled ? { cursor: "default" } : { cursor: "text" }} onClick={handleClick}>
          {props.dialCode}
        </div>
        <input
      ...
      <div className="focusBorder" style={style.inputFocused ? { width: "100%" } : { width: "0%" }} />
      <label className="floatingLabel" style={{ top: "0%", color: style.labelColour, fontSize: "12px" }}>
        {props.label}
      </label>
    </div>
  );
