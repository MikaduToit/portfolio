...
const verifyJWT = (req, res, next) => {
  ...
  if (!authHeader?.startsWith("Bearer ")) return res.status(403).json({ message: "Unauthorized request!" }); //FORBIDDEN, because request header does not contain an Access Token.
